...
// Guards RF-004 / R-001 (docs/specs/001): a previously published slug must
// never disappear from packages/content/posts. Deliberately dependency-free
// and doesn't boot Astro's content API — slugs are just filenames (minus
// extension), same rule the content loader itself uses.

const __dirname = dirname(fileURLToPath(import.meta.url));
const postsDir = resolve(__dirname, "..", "..", "..", "packages", "content", "posts");
const fixturePath = resolve(__dirname, "..", "test", "fixtures", "published-slugs.json");
...
if (missing.length > 0) {
  console.error(
    `Slug regression detected: the following previously published slug(s) are missing from ${postsDir}:\n` +
      ...
      "\n\nA published slug is immutable (RF-004). Restore the file or rename it back before building."
  );
  process.exit(1);
}
